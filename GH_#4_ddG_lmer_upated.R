...
Compare_0h = full_join(Input_0h %>% mutate(Pos=as.numeric(Pos)),
                       Total_ddG) %>% na.omit()
Compare_48h = full_join(clean_48h %>% mutate(Pos=as.numeric(Pos)),
                        Total_ddG) %>% na.omit()

# Function for creating box plots and performing mixed-effects analysis
create_boxplot_with_anova <- function(data, x_var, y_var, pos_var, title) {
  ...
  lmer_result <- lmer(formula, data = data)
  print(lmer_result)
  
  # Obtain the ANOVA summary from the mixed-effects model
  anova_result <- anova(lmer_result)
  
  # Extract p-value for the group effect
  p_value <- anova_result$`Pr(>F)`[1]  # Assuming group is the first fixed effect
  print(p_value)
  ...
  p <- ggplot(data, aes(x = factor(group), y = get(y_var), color = group)) +
    ...
    ggtitle(title) +
    theme_classic() +
    labs(x = "Group", y = y_var, color = "Group") +
