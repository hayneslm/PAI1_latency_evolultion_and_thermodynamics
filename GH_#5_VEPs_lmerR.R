...
create_boxplot_with_anova <- function(data, x_var, y_var, pos_var, title) {
  ...
  z_value <- qnorm(0.975)  # Critical value for 95% confidence interval
  calc_summary$CI_lower <- calc_summary$mean - z_value * calc_summary$se
  calc_summary$CI_upper <- calc_summary$mean + z_value * calc_summary$se
  
  # Perform mixed-effects model using lmer to incorporate random effects
  formula <- as.formula(paste(y_var, "~ group + (1 |", pos_var, ")"))
  # formula <- as.formula(paste(y_var, " ~ group", ))
  lmer_result <- lmerTest::lmer(formula, data = data)
  print(lmer_result)
  
  # Obtain p-value for the group effect
  anova_summary <- anova(lmer_result)
  print(summary(lmer_result))
  print(anova_summary)
  p_value <- anova_summary$`Pr(>F)`[1]  # Assuming the group is the first fixed effect
  print(p_value)
  
  # Generate the boxplot with jittered points and annotations for CI
  ...
  # Create a summary data frame for ANOVA results including CIs
  anova_summary_df <- data.frame(
    ...
    Group1_CI_Lower = calc_summary$CI_lower[calc_summary$group == "<=0"],
    Group1_CI_Upper = calc_summary$CI_upper[calc_summary$group == "<=0"],
    Group2_CI_Lower = calc_summary$CI_lower[calc_summary$group == ">0"],
    Group2_CI_Upper = calc_summary$CI_upper[calc_summary$group == ">0"],
    p_value = p_value,
    Group1_N = calc_summary$n[calc_summary$group == "<=0"],
    Group2_N = calc_summary$n[calc_summary$group == ">0"]
  )
  
  return(list(plot = p, anova_result = anova_summary_df))
}
...
# Combine plots
plots <- list(
  result_eve_0h$plot, result_am_0h$plot, result_cpt_0h$plot,
  result_eve_48h$plot, result_am_48h$plot, result_cpt_48h$plot
)
...
anova_summaries <- bind_rows(
  result_eve_0h$anova_result %>% mutate(source = "result_eve_0h"),
  result_am_0h$anova_result %>% mutate(source = "result_am_0h"),
  result_cpt_0h$anova_result %>% mutate(source = "result_cpt_0h"),
  result_eve_48h$anova_result %>% mutate(source = "result_eve_48h"),
  result_am_48h$anova_result %>% mutate(source = "result_am_48h"),
  result_cpt_48h$anova_result %>% mutate(source = "result_cpt_48h")
)
